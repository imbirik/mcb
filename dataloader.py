...
LM1B_DATASET_REPO = 'jdeschena/lm1b'
...
def get_dataset(dataset_name,
    ...
    if utils.fsspec_exists(_path):
        ...
        try:
            ...
            LOGGER.warning(
                'Failed to load cached dataset at %s. Regenerating cache. '
                'Original error: %s',
                _path,
                exc)
    ...
    if dataset_name == 'wikitext103':
        ...
        LOGGER.info(
            'Loading LM1B from parquet mirror %s because the legacy '
            'Hugging Face dataset script is no longer supported.',
            LM1B_DATASET_REPO)
        dataset = datasets.load_dataset(
            LM1B_DATASET_REPO,
            cache_dir=cache_dir,
