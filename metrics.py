...
def compute_text_diversity(text_samples, n_values=(2, 3, 4)):
    """Compute corpus-level distinct-n statistics and their product.

    This follows the standard distinct-n family used for open-ended text
    diversity reporting: unique n-grams divided by total n-grams.
    """
    distinct_scores = {}
    product = 1.0
...
def compute_mauve_score(
        ...
        batch_size=8,
        verbose=False):
    """Compute MAUVE between reference and generated corpora."""
    if len(reference_texts) == 0 or len(generated_texts) == 0:
        return None
    ...
    try:
        ...
        raise ModuleNotFoundError(
            'MAUVE metric requested but `mauve-text` is not installed.'
        ) from exc

    num_samples = min(len(reference_texts), len(generated_texts))
    out = mauve.compute_mauve(
        p_text=reference_texts[:num_samples],
        q_text=generated_texts[:num_samples],
        device_id=device_id,
